...
const start = async () => {
  const maxRetries = Number(process.env.DB_INIT_RETRIES ?? 12);
  const retryDelay = Number(process.env.DB_INIT_DELAY ?? 5000);
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
    try {
      ...
      const message = "Impossible de démarrer le serveur (DB KO)";
      console.error(message, error.message);
      if (attempt === maxRetries) {
        process.exit(1);
      }
      console.log(`Nouvelle tentative dans ${retryDelay} ms (${attempt + 1}/${maxRetries + 1})`);
